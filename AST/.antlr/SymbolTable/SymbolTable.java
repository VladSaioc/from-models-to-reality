...
public class SymbolTable 
{
	...
	private HashMap<String, Stack<SymbolDetails>> softTable;
	...
	public void addSymbol(String name, String type)
	{
		...
		{
			softTable.get(name).push(new SymbolDetails(type, scopeLevel));
		}
		else
		{
			Stack<SymbolDetails> newStack=new Stack<SymbolDetails>();
			newStack.push(new SymbolDetails(type, scopeLevel));
	...
	public void addFunctionSymbol(String name, String functionType, ArrayList<String> paramTypes)
	{
		...
		{
			Stack <SymbolDetails> newStack=new Stack<SymbolDetails>();
			SymbolDetails functionEntry=new SymbolDetails(functionType, scopeLevel);
			functionEntry.addParameterTypes(paramTypes);
	...
	public void closeSoftScope()
	{
		if (scopeLevel>0)
		{
			...
			{
				if (softTable.get(name).peek().scopeLevel==scopeLevel)
				{
					removeSymbol(name);
				}
			}
	...
	public void addArraySymbol(String name, String type, int count)
	{
		if (softTable.containsKey(name))
		{
			softTable.get(name).push(new SymbolDetails(type, count, scopeLevel));
		}
		else
		{
			Stack<SymbolDetails> newStack=new Stack<SymbolDetails>();
			newStack.push(new SymbolDetails(type, count, scopeLevel));
			softTable.put(name, newStack);
		}
	}
}
